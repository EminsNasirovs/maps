...
// Function to convert LKS-92 GeoJSON to WGS84
function convertLKS92ToWGS84(geojsonData) {
    geojsonData.features.forEach(feature => {
        if (feature.geometry.type === 'Point') {
            const [x, y] = feature.geometry.coordinates;
            const [lat, lon] = LKS92WGS84.convertXYToLatLon([x, y]);
            feature.geometry.coordinates = [lon, lat];
        } else if (feature.geometry.type === 'Polygon' || feature.geometry.type === 'MultiPolygon') {
            feature.geometry.coordinates = feature.geometry.coordinates.map(ring =>
                ring.map(([x, y]) => LKS92WGS84.convertXYToLatLon([x, y]).reverse())
            );
        }
    });
    return geojsonData;
...
fetch('locations.json')
    ...
    })  // Closing the second "then"
    .catch(error => console.error('Error loading GeoJSON:', error));  // Catch errors
